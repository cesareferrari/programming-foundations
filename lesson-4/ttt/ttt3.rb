...
def danger_line(brd)
  WINNING_LINES.each { |line| return line if threat_line?(brd, line) }
  nil
end
...
def computer_places_piece!(brd)
  ...
  if danger
    sub_board = brd.select { |key, _| danger.include? key }
    square = sub_board.select { |_, value| value == INITIAL_MARKER }.keys.first
  else
